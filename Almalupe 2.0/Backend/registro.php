	...
	$confirmacion_existencia_cuenta = false;
		...
		if(isset($_POST['register']))
		{
				...
				if (!$row2 == "")
				{
					if ($row2['email'] == $email || $row2['username'] == $username)
					{
						if ($row2['email'] == $email)
						{
							...
							window.location.href="../Paginas/signup_login/signup.html";</script><?php
						}
			...
			if ($confirmacion_email == true && $confirmacion_password == true)
			{
				...
				{
					if ($confirmacion_date == true && $confirmacion_sex == true)
					{
						if ($confirmacion_existencia_cuenta == true)
						{
							//$password = md5($password);

							$consulta = "INSERT INTO usuarios(email, username, password, birth_date, sex, rol_id, fecha_reg) 
							VALUES ('$email','$username','$password','$fecha_completa','$sex', '7', '$ultima_conexion')";
							...
							if ($resultado)
							{
								...
								$consulta_comprobacion = "SELECT username, password, rol_id FROM usuarios WHERE username = '$username' AND password = '$password'";
								$resultado_comprobacion = mysqli_query($conex, $consulta_comprobacion);

								$row_comprobacion = $resultado_comprobacion->fetch_assoc();

								if ($row_comprobacion['username'] == $username && $row_comprobacion['password'] == $password)
								{		
									$consulta_conexion  = "UPDATE usuarios SET ult_conexion = '$ultimaConexion' WHERE username = '$username' AND password = '$password'";
